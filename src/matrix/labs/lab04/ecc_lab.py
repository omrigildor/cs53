...
def find_error(e):
    ...
    return Vec({0,1,2,3,4,5,6}, {i:one for i in range(7) if e == H_col[i]})

print(find_error(Vec({0,1,2}, {})))
...
def find_error_matrix(S):
    ...
    return matutil.coldict2mat({i:find_error(matutil.mat2coldict(S)[i]) for i in matutil.mat2coldict(S).keys()})
