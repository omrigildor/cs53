...
def is_echelon(A):
    ...
    index_of_last_non_zero = -1

    for i in A:
        g = [x for x in range(len(i)) if i[x] != 0]
        if len(g) == 0:
            index_of_last_non_zero = len(i)
        elif (g[0]) <= index_of_last_non_zero:
            return False
        else:
            index_of_last_non_zero = g[0]

    return True

print(is_echelon([[1,1,1],[0,1,1],[0,0,1]]), "answer")
print(is_echelon([[0,1,1],[0,1,0],[0,0,1]]))
print(is_echelon([[1,1]]),True)
print(is_echelon([[1]]),True)
print(is_echelon([[1],[1]]),False)
print(is_echelon([[0]]), True)
...
print([is_echelon(M) for M in [M1, M2, M3, M4, M5, M6]])
