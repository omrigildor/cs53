...
your_answer_b_BA = [[0,0,0,0],[4,4,4,0],[0,0,0,0],[0,0,0,0]]

your_answer_c_AB = [[6,0,0,0],[6,0,0,0],[8,0,0,0],[5,0,0,0]]
your_answer_c_BA = [[0,0,0,0],[1,5,-2,3],[0,0,0,0],[4,4,4,0]]
...
def lin_comb_mat_vec_mult(M, v):
    ...
    return sum([v[x] * M_col[x] for x in v.D])
...
def lin_comb_vec_mat_mult(v, M):
    ...
    Examples:
      ...
      >>> M1=Mat(({'a','b'},{0,1}), {('a',0):8, ('a',1):2, ('b',0):-2, ('b',1):1})
      >>> v1=Vec({'a','b'},{'a':4,'b':3})
      >>> lin_comb_vec_mat_mult(v1,M1)
      Vec({0, 1},{0: 26, 1: 11})
    '''
    assert(v.D == M.D[0])
    M_col = matutil.mat2rowdict(M)
    return sum([v[x] * M_col[x] for x in v.D])
...
def dot_product_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    M_col = matutil.mat2coldict(M)
    return Vec(M.D[1], {i:(v * M_col[i]) for i in M.D[1]})
...
# print(Mv_mat_mat_mult(A, B), "weow")
# print(Mv_mat_mat_mult(A, B) == Mat(({0,1,2}, {0,1,2}), {(0,0):15, (0,1):12, (1,0):25, (1,1):31}), "really?")
#
# C = Mat(({0,1,2}, {'a','b'}), {(0,'a'):4, (0,'b'):-3, (1,'a'):1, (2,'a'):1, (2,'b'):-2})
# D = Mat(({'a','b'}, {'x','y'}), {('a','x'):3, ('a','y'):-2, ('b','x'):4, ('b','y'):-1})
# print(Mv_mat_mat_mult(C,D) == Mat(({0,1,2}, {'x','y'}), {(0,'y'):-5, (1,'x'):3, (1,'y'):-2, (2,'x'):-5}))

## 15: (Problem 4.17.18) Vector-matrix matrix-matrix multiply
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return matutil.rowdict2mat({i:(v*B) for i,v in matutil.mat2rowdict(A).items()})

#
# print(vM_mat_mat_mult(A,B), "weow")
# print(vM_mat_mat_mult(A,B) == Mat(({0,1,2}, {0,1,2}), {(0,0):15, (0,1):12, (1,0):25, (1,1):31}), "really?")
